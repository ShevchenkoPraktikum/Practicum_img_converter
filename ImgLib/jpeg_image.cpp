...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    vector<JSAMPLE> row_buffer(width * 3);
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        for (int x = 0; x < width; ++x) {
            row_buffer[x * 3 + 0] = static_cast<JSAMPLE>(to_integer<int>(line[x].r));
            row_buffer[x * 3 + 1] = static_cast<JSAMPLE>(to_integer<int>(line[x].g));
            row_buffer[x * 3 + 2] = static_cast<JSAMPLE>(to_integer<int>(line[x].b));
        }
...
void SaveSсanlineToImage(const JSAMPLE* row, int y, Image& out_image) {
    ...
    for (int x = 0; x < out_image.GetWidth(); ++x) {
        ...
        line[x] = Color{
            byte{static_cast<unsigned char>(pixel[0])},
            byte{static_cast<unsigned char>(pixel[1])},
            byte{static_cast<unsigned char>(pixel[2])},
            byte{255}
        };
    }
}
